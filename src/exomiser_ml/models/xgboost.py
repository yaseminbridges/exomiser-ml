...
def xgboost(training_data: Path, test_dir: Path, features: List[str], output_dir: Path) -> None:
    model = train_model(training_data, features)
    test_model(test_dir, model, features, output_dir)


def run_xgboost_pipeline(phenopacket_dir: Path, result_dir: Path, output_dir: Path, features: List[str],
                                     test_size: float):
    ...
    xgboost(training_data=output_dir.joinpath("results_split/train/train.tsv"),
...
def run_xgboost(training_data: Path, test_dir: Path, features: List[str], output_dir: Path,
                            phenopacket_dir: Path) -> None:
    output_dir.joinpath("pheval_variant_results").mkdir(parents=True, exist_ok=True)
    output_dir.joinpath("raw_results").mkdir(parents=True, exist_ok=True)
    xgboost(training_data, test_dir, list(features), output_dir.joinpath("raw_results"))
    post_process_test_dir(test_dir=output_dir.joinpath("raw_results"), phenopacket_dir=phenopacket_dir,
                          output_dir=output_dir)
    metadata = RunMetadata(
        test_size=None,
...
@click.command("run-xgb-pipeline")
@click.option('--phenopacket-dir', "-p", type=Path, required=True, help="Path to the Phenopacket data directory.")
