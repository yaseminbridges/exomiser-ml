...
export_json_option = click.option(
    '--export-json', '-e', type=Path, required=True,
...
@click.command("run-ebm")
@training_data_option
@test_dir_option
@features_option
@output_dir_option
@phenopacket_dir_option
def run_ebm(training_data: Path, test_dir: Path, features: List[str], output_dir: Path, phenopacket_dir: Path):
    train_and_test_gam(training_data=training_data, test_dir=test_dir, features=features, output_dir=output_dir,
...
@filter_bs4_option
@filter_pp4_option
def add_features_command(phenopacket_dir: Path, result_dir: Path, output_dir: Path, filter_clinvar: bool,
                         filter_bs4: bool, filter_pp4: bool) -> None:
